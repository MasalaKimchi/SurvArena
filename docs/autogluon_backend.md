...
`manuscript_v1.yaml`) with `--method` and `--dataset` as needed. No maintained
AutoGluon-only benchmark config is present in this checkout, so AutoGluon
appendix claims need a separate bounded config and evidence bundle.
