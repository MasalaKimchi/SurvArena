...
class TabPFNSurvivalMethod(BaseSurvivalMethod):
    ...
    def _extract_inference_embeddings(self, X: np.ndarray) -> np.ndarray:
        ...
        if embeddings.ndim == 2:
            features = embeddings
        elif str(self.params["aggregate_estimators"]) == "concat":
    ...
    def fit(
        ...
        try:
            ...
            backbone_training = str(self.params["backbone_training"]).lower()
            if backbone_training not in {"frozen", "finetune"}:
                raise ValueError("backbone_training must be one of {'frozen', 'finetune'}.")
...
class TabPFNHorizonSurvivalMethod(BaseSurvivalMethod):
    def __init__(
        self,
        n_estimators: int = 4,
        fit_mode: str = "fit_preprocessors",
    ...
    def fit(
        ...
        ensure_foundation_runtime_ready("tabpfn_survival_horizon", checkpoint_path=self.params.get("checkpoint_path"))
        try:
            X_train_np = np.asarray(X_train, dtype=np.float32)
            time_train_np = np.asarray(time_train, dtype=np.float64)
            event_train_np = np.asarray(event_train, dtype=np.int32)
            if int(event_train_np.sum()) <= 0:
                raise ValueError("TabPFN horizon survival training requires at least one observed event.")
    ...
    def _horizon_event_probabilities(self, X: np.ndarray) -> np.ndarray:
        if self.horizon_times_ is None or self.constant_event_probabilities_ is None:
            raise RuntimeError("TabPFNHorizonSurvivalMethod must be fit before prediction.")
        X_np = np.asarray(X, dtype=np.float32)
