...
from survarena.methods.foundation.neural_cox import forward_in_chunks, train_neural_cox_head
...
class MitraSurvivalMethod(BaseSurvivalMethod):
    ...
    def predict_risk(self, X: np.ndarray) -> np.ndarray:
        ...
        features_t = torch.as_tensor(features, dtype=torch.float32, device=self.device_)
        self.head.eval()
        with torch.no_grad():
            risk = forward_in_chunks(self.head, features_t, batch_size=int(self.params["batch_size"])).detach().cpu().numpy()
